...
exports.createArticleToken = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ data: newArticle, message: "Article added successfully" });
  } catch (error) {
    console.log(error);
...
exports.updateArticle = async (req, res) => {
  try {
    ...
    if (parseInt(userId) !== authenticatedUserId) {
      return res
        .status(403)
        .json({ message: "Forbidden, you can only update your own articles" });
    }
    ...
    return res
      .status(200)
      .json({ data: articleToUpdate, message: "Article updated successfully" });
  } catch (error) {
    return handleServerError(res);
  }
};
...
exports.deleteArticleToken = async (req, res) => {
  try {
    ...
    if (articleToDelete.photoArticle) {
      const imagePath = articleToDelete.photoArticle.split(
        "http://localhost:3001"
      )[1];
